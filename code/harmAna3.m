...
[HarmResult1, FundaResult1] = anaHarm(fftWinDataFirst)
[HarmResult2, FundaResult2] = anaHarm(fftWinDataSecond)


% 画图，画出直接fft的相位谱和修正后的相位谱，看起来差不多，具体咋修正的就是直接抄的文章
index = linspace(1, length(HarmResult1), length(HarmResult1));
figure(1);
plot(index, HarmResult1(4,:), '-r');
rawAngle = angle(fft(dataFirst(2, :)));
hold on;
plot(index, rawAngle(HarmResult1(1, :)), '--b');

index = linspace(1, length(HarmResult2), length(HarmResult2));
figure(2);
plot(index, HarmResult2(4,:), '-r');
rawAngle = angle(fft(dataSecond(2, :)));
hold on;
plot(index, rawAngle(HarmResult2(1, :)), '--b');
...
function [HarmResult, FundaResult] = anaHarm(fftWinData)
    ...
    % 获取原始相位
    rawAngle = angle(fftWinData);
    % 计算修正后的谐波相位
    harmAngle = rawAngle(harmIndex(2, :)) + 0.5*pi - delta.*pi;
    % 计算修正后的基波相位
    fundaAngle = rawAngle(RealfundaIndex(2, :)) + 0.5*pi - fDelta.*pi;
